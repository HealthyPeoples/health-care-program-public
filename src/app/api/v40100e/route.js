/**
 * @file API /api/v40100e — 월급여 상세 뷰 V40100E
 *
 * @description
 * 월급여 상세 뷰 V40100E Next.js Route Handler. 세션 ANCD 게이트·MSSQL 직접 접근 패턴을 따릅니다.
 *
 * @module app/api/v40100e/route
 */
import { connPool } from '../../../config/server';
import { assertAnCdMatchesSession } from '../../../config/sessionServer';

import { jsonOk, jsonError } from '../../../utils/apiResponse';
const sql = require('mssql');

const { normalizeYmdEmptyRaw: normalizeYmd } = require('../../../utils/normalizeYmd');
const VIEW = '[돌봄시설DB].[dbo].[V40100E]';

function normalizeSalmm(v) {
	if (v == null || v === '') return null;
	const s = String(v).replace(/\D/g, '');
	if (s.length === 6) return s;
	return null;
}

function num(v) {
	const n = parseInt(String(v ?? '0').replace(/,/g, ''), 10);
	return Number.isFinite(n) ? n : 0;
}


function mapRow(r) {
	return {
		ANCD: r.ANCD,
		SALMM: r.SALMM != null ? String(r.SALMM).trim() : '',
		SALYY: r.SALYY != null ? String(r.SALYY).trim() : '',
		MM: r.MM != null ? String(r.MM).trim() : '',
		periodFrom: normalizeYmd(r['시작일자']),
		periodTo: normalizeYmd(r['종료일자']),
		ANGH: r.ANGH != null ? String(r.ANGH).trim() : '',
		PNUM: r.PNUM != null ? String(r.PNUM).trim() : '',
		recipient: r['수급자'] != null ? String(r['수급자']).trim() : '',
		recognitionNo: r['인정번호'] != null ? String(r['인정번호']).trim() : '',
		birthday: normalizeYmd(r['수급자생일']),
		sex: r['성별'] != null ? String(r['성별']).trim() : '',
		orgCode: r['기관기호'] != null ? String(r['기관기호']).trim() : '',
		orgName: r['기관명'] != null ? String(r['기관명']).trim() : '',
		orgAddr: r['주소'] != null ? String(r['주소']).trim() : '',
		orgBizNo: r['사업번호'] != null ? String(r['사업번호']).trim() : '',
		orgOwner: r['대표자명'] != null ? String(r['대표자명']).trim() : '',
		orgTel: r['센터전화번호'] != null ? String(r['센터전화번호']).trim() : '',
		bankAccount: r['입금통장'] != null ? String(r['입금통장']).trim() : '',
		otherCostDesc: r['기타비용내역'] != null ? String(r['기타비용내역']).trim() : '',
		daysUsed: num(r['사용일수']),
		nhaContribution: num(r['공단부담금']),
		recipientContribution: num(r['수급자부담금']),
		mealFee: num(r['식대']),
		nonBenefitSnack: num(r['비급여간식']),
		nonBenefitMedical: num(r['비급여의료비']),
		beautyCost: num(r['이미용']),
		roomUpgradeFee: num(r['병실승급비']),
		contractedMedical: num(r['촉탁의료비']),
		contractedPrescription: num(r['촉탁처방비']),
		otherCost: num(r['기타비용']),
		monthLastDay: normalizeYmd(r['월마지막일자']),
		issueDate: normalizeYmd(r['발행일자']),
	};
}

/**
 * V40100E 월별 급여명세서 뷰
 * GET /api/v40100e?salmm=YYYYMM
 * GET /api/v40100e?salmm=YYYYMM&pnum=PNUM
 * GET /api/v40100e?salmm=YYYYMM&pnums=1,2,3
 */
export async function GET(req) {
	try {
		const sp = req.nextUrl.searchParams;
		const ancd = sp.get('ancd');
		const gate = assertAnCdMatchesSession(req, ancd || null);
		if (!gate.ok) return gate.response;

		const salmm = normalizeSalmm(sp.get('salmm'));
		if (!salmm) {
			return jsonError({ success: false, error: 'salmm(YYYYMM) 파라미터가 필요합니다.' }, 400);
		}

		const pool = await connPool;
		if (!pool) {
			return jsonError({ success: false, error: '데이터베이스 연결 실패' });
		}

		const request = pool.request();
		request.input('sessionAncd', gate.sessionAncd);
		request.input('salmm', sql.Char(6), salmm);

		const pnum = sp.get('pnum');
		const pnumsRaw = sp.get('pnums');

		let result;
		if (pnum != null && String(pnum).trim() !== '') {
			request.input('pnum', sql.VarChar(30), String(pnum).trim());
			result = await request.query(`
        SELECT TOP 1 *
        FROM ${VIEW}
        WHERE [ANCD] = @sessionAncd
          AND LTRIM(RTRIM([SALMM])) = LTRIM(RTRIM(@salmm))
          AND CAST([PNUM] AS VARCHAR(30)) = @pnum
      `);
			const row = result.recordset?.[0] ? mapRow(result.recordset[0]) : null;
			return jsonOk({ success: true, data: row, salmm });
		}

		if (pnumsRaw != null && String(pnumsRaw).trim() !== '') {
			const list = String(pnumsRaw)
				.split(',')
				.map((s) => s.trim())
				.filter(Boolean);
			if (list.length === 0) {
				return jsonOk({ success: true, data: [], count: 0, salmm });
			}
			const placeholders = list
				.map((_, i) => {
					request.input(`p${i}`, sql.VarChar(30), list[i]);
					return `@p${i}`;
				})
				.join(',');
			result = await request.query(`
        SELECT *
        FROM ${VIEW}
        WHERE [ANCD] = @sessionAncd
          AND LTRIM(RTRIM([SALMM])) = LTRIM(RTRIM(@salmm))
          AND CAST([PNUM] AS VARCHAR(30)) IN (${placeholders})
        ORDER BY [수급자] ASC, CAST([PNUM] AS VARCHAR(30)) ASC
      `);
		} else {
			result = await request.query(`
        SELECT *
        FROM ${VIEW}
        WHERE [ANCD] = @sessionAncd
          AND LTRIM(RTRIM([SALMM])) = LTRIM(RTRIM(@salmm))
        ORDER BY [수급자] ASC, CAST([PNUM] AS VARCHAR(30)) ASC
      `);
		}

		const seen = new Set();
		const mapped = [];
		for (const raw of result.recordset || []) {
			const key = String(raw.PNUM ?? '').trim();
			if (!key || seen.has(key)) continue;
			seen.add(key);
			mapped.push(mapRow(raw));
		}

		return jsonOk({
				success: true,
				data: mapped,
				count: mapped.length,
				salmm,
			});
	} catch (err) {
		console.error('V40100E GET 오류:', err);
		return jsonError({ success: false, error: err.message, details: String(err) });
	}
}
