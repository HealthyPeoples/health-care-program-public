/**
 * @file API /api/f60010/photos — 직원회의록 사진
 *
 * @description
 * F60010.MIMG에 저장할 blob 경로를 업로드·조회·삭제합니다.
 *
 * @module app/api/f60010/photos/route
 */
...
function assertBlobPathOwnedBySession(blobName, sessionAncd) {
  ...
  const prefix = `f60010/${String(sessionAncd).trim()}/`;
...
export async function GET(req) {
  try {
    ...
    console.error('직원회의록 사진 조회 오류:', err);
...
export async function POST(req) {
  try {
    ...
    const uploaded = await uploadEmployeeMeetingPhoto({
    ...
    const viewUrl = `/api/f60010/photos?blobName=${encodeURIComponent(uploaded.blobName)}`;
    ...
    console.error('직원회의록 사진 업로드 오류:', err);
...
export async function DELETE(req) {
  try {
    ...
    console.error('직원회의록 사진 삭제 오류:', err);
