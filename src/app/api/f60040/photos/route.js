/**
 * @file API /api/f60040/photos — 보호자간담회 사진
 *
 * @description
 * F60040.MIMG(nvarchar(100))에 저장할 blob 경로를 업로드·조회·삭제합니다.
 *
 * @module app/api/f60040/photos/route
 */
...
function assertBlobPathOwnedBySession(blobName, sessionAncd) {
  ...
  const prefix = `f60040/${String(sessionAncd).trim()}/`;
...
export async function GET(req) {
  try {
    ...
    console.error('보호자간담회 사진 조회 오류:', err);
...
export async function POST(req) {
  try {
    ...
    const uploaded = await uploadGuardianMeetingPhoto({
    ...
    if (String(uploaded.blobName).length > MIMG_MAX_LEN) {
      return jsonError({ success: false, error: '사진 경로가 저장 길이를 초과합니다.' }, 400);
    }

    const viewUrl = `/api/f60040/photos?blobName=${encodeURIComponent(uploaded.blobName)}`;
    ...
    console.error('보호자간담회 사진 업로드 오류:', err);
...
export async function DELETE(req) {
  try {
    ...
    console.error('보호자간담회 사진 삭제 오류:', err);
