/**
 * @file API /api/f33010/photos — 욕창관리 사진
 *
 * @description
 * F33010.DCUB_IMG에 저장할 blob 경로를 업로드·조회·삭제합니다.
 *
 * @module app/api/f33010/photos/route
 */
...
function assertBlobPathOwnedBySession(blobName, sessionAncd) {
  ...
  const prefix = `f33010/${String(sessionAncd).trim()}/`;
...
export async function GET(req) {
  try {
    ...
    console.error('욕창관리 사진 조회 오류:', err);
...
export async function POST(req) {
  try {
    ...
    const uploaded = await uploadBedsorePhoto({
    ...
    const viewUrl = `/api/f33010/photos?blobName=${encodeURIComponent(uploaded.blobName)}`;
    ...
    console.error('욕창관리 사진 업로드 오류:', err);
...
export async function DELETE(req) {
  try {
    ...
    console.error('욕창관리 사진 삭제 오류:', err);
