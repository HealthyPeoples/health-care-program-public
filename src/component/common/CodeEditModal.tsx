...
export default function CodeEditModal({ open, onClose }: CodeEditModalProps) {
  if (!open) return null;
  return (
    <div
    ...
    >
      ...
      >
        {/* 상단 타이틀 */}
        <div className="bg-blue-200 border border-blue-400 rounded text-center py-4 mb-4 text-2xl font-semibold text-blue-900">일반 코드 수정</div>
        {/* 입력 폼 */}
        <div className="space-y-2 mb-4">
          <div className="flex items-center">
            <label className="w-24 bg-blue-100 border border-blue-400 px-2 py-1 text-base text-blue-900">코드구분</label>
            <input className="flex-1 border border-blue-400 px-2 py-1 ml-2 rounded bg-white text-black" value="AA" readOnly />
          </div>
          <div className="flex items-center">
            <label className="w-24 bg-blue-100 border border-blue-400 px-2 py-1 text-base text-blue-900">설 명</label>
            <input className="flex-1 border border-blue-400 px-2 py-1 ml-2 rounded bg-white text-black" value="환자정보_요양등급" readOnly />
          </div>
          <div className="flex items-center">
            <label className="w-24 bg-blue-100 border border-blue-400 px-2 py-1 text-base text-blue-900">비 고</label>
            <input className="flex-1 border border-blue-400 px-2 py-1 ml-2 rounded bg-white text-black" value="" readOnly />
          </div>
        </div>
        {/* 버튼 영역 */}
