...
export type IndividualVolunteerPrintData = {
	facilityName: string;
...
function sharedStyles(): string {
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .roster-table .no { width: 10%; }
    .roster-table .date { width: 22%; }
    .roster-table .time { width: 28%; }
    .roster-table .name { width: 20%; }
    .roster-table .sign { width: 20%; }
    .footer {
      margin-top: 18px;
      text-align: right;
      font-size: 10.5pt;
...
function buildContentPageBody(data: IndividualVolunteerPrintData): string {
	...
	return `
    <div class="doc-title">개인 봉사활동 내용</div>
    <div class="sub-info">
    ...
    <table>
      <tr>
        <td class="lbl">봉사자</td>
        <td class="val">${nbsp(data.volunteerName)}</td>
        <td class="lbl">연락처</td>
        <td class="val">${nbsp(data.phone)}</td>
      </tr>
      <tr>
        <td class="lbl">주소</td>
        <td class="val" colspan="3">${nbsp(data.address)}</td>
      </tr>
      <tr>
        <td class="lbl">봉사일자</td>
        <td class="val">${nbsp(data.date)}</td>
    ...
    <div class="footer">
      위와 같이 개인 봉사활동을 실시하였음을 확인합니다.<br/>
      ${escapeHtml(data.date || new Date().toISOString().slice(0, 10))}<br/><br/>
      담당자 : ________________&nbsp;&nbsp;(인)
    </div>`;
}

export function buildIndividualVolunteerContentBatchPrintHtml(
	list: IndividualVolunteerPrintData[]
): string {
	const pages = list
		.map(
			(data, idx) =>
...
/** 체크된 봉사자 명단 — 자원봉사자(개인) 명단 양식 */
export type IndividualVolunteerListPrintRow = {
	name: string;
	phone: string;
	address: string;
	etc: string;
...
export function buildIndividualVolunteerListPrintHtml(
	rows: IndividualVolunteerListPrintRow[],
	baseDate: string
): string {
	...
	const bodyRows = rows
      ...
      <td class="c-addr">${nbsp(r.address)}</td>
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .c-name { width: 14%; text-align: left; }
    .c-phone { width: 18%; text-align: left; }
    .c-addr { width: 36%; text-align: left; }
    .c-etc { width: 16%; text-align: left; }
...
<body>
  <div class="page">
    <div class="header">
      <div class="doc-title">자원봉사자(개인) 명단</div>
