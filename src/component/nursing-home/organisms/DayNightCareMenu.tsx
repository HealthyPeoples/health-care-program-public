...
/**
 * @file 요양원 UI — DayNightCareMenu.tsx
 *
 * @description
 * 요양원 공통 UI(메뉴·패널 등) 컴포넌트입니다.
 *
 * @module component/nursing-home/organisms/DayNightCareMenu
 */
import React, { useMemo, useState } from 'react';
import Link from 'next/link';
import { menuNameMatchesQuery, type Sections3 } from '../menuData';

// 주야간보호 메뉴 데이터 (추후 확장 가능)
const dayNightCareMenuData: Sections3 = {
  '주야간보호 관리': {
    '기본 관리': [
      { name: '회원 정보 관리', link: '/dayNightCare/member-info' },
      { name: '서비스 제공 내역', link: '/dayNightCare/service-record' },
    ],
  },
};

export default function DayNightCareMenu() {
  const [openSection, setOpenSection] = useState<string | null>(null);
  const [openGroupKey, setOpenGroupKey] = useState<string | null>(null);
  const [query, setQuery] = useState('');

  const filtered = useMemo(() => {
    if (!query.trim()) return dayNightCareMenuData;
    const q = query.trim();
    const result: Sections3 = {};
    Object.entries(dayNightCareMenuData).forEach(([sectionTitle, groupsObj]) => {
  ...
  return (
    <nav className="w-full min-w-0 h-full bg-white p-4">
      {/* 메뉴 검색 */}
      <div className="mb-3">
        <input
          value={query}
